...
- **읽기 작업만 하더라도 트랜잭션을 거는 것이 좋다.** 트랜잭션을 걸지 않으면 모든 SELECT 쿼리마다 commit을 하기 때문에 성능이 떨어진다. 명시적으로 트랜잭션을 걸어주면 마지막에 한 번만 commit하면 되어 성능이 향상된다. 이러한 읽기 전용 최적화에 대한 더 자세한 내용은 [[JPA-성능-최적화]]에서 확인할 수 있다.
