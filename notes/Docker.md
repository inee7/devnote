...
도커는 프로세스 격리를 통해 가볍고 이식성 높은 환경을 제공하며, Dockerfile의 다양한 명령어로 이미지 생성과 컨테이너 실행을 자동화한다.

## 핵심 정리

- **Docker vs. VM**: 도커는 프로세스 가상화, VM은 하드웨어 가상화. 도커가 더 가볍고 빠르며 이식성이 높다.
- **Dockerfile**: 이미지 생성을 위한 명세서. `RUN`, `CMD`, `ENTRYPOINT`, `COPY`, `VOLUME` 등의 명령어를 사용해 환경을 코드화.
- **`CMD` vs. `ENTRYPOINT`**: `CMD`는 `docker run` 시 명령어로 쉽게 대체 가능 (기본값 제공), `ENTRYPOINT`는 컨테이너의 주 실행 명령을 고정하고 인자만 받을 때 유용.
- **`COPY` vs. `ADD`**: `COPY`는 단순 파일 복사, `ADD`는 URL 다운로드 및 압축 해제 기능까지 포함.
- **데이터 영속성**: 컨테이너는 휘발성이므로, 데이터 보존을 위해서는 `VOLUME`을 사용해야 한다.
