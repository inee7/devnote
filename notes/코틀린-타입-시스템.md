...
```
/* 배열 사용하기 */fun main(args: Array<String>) {    for (i in args.indices) {        println("Argument ${i} is: ${args[i]}")    }}/* 알파벳으로 이루어진 배열 만들기 */>>> val letters = Array<String>(26) { i -> ('a' + i).toString() }>>> println(letters.joinToString(""))abcdefghijklmnopqrstuvwxyz
```
